...
const miniAppInput = z.object({
  ...
  features: z
    ...
    .describe("List of core features / modules (e.g. 'search bar', 'results list', 'filter options')"),
  theme: z
...
function generateFeatureComponent(feature: string, index: number): any {
  ...
  if (featureLower.includes("input") || featureLower.includes("search") || featureLower.includes("text")) {
    return {
      type: "input",
      props: {
      ...
      state: [`const [inputValue${index}, setInputValue${index}] = useState('');`],
      handlers: [
  ...
  if (featureLower.includes("button") || featureLower.includes("submit") || featureLower.includes("action")) {
    return {
      type: "button",
      props: {
  ...
  if (featureLower.includes("list") || featureLower.includes("results") || featureLower.includes("items")) {
    return {
      type: "list",
      props: {
        items: `items${index}`,
      },
      state: [`const [items${index}, setItems${index}] = useState<string[]>(['Item 1', 'Item 2', 'Item 3']);`],
    };
  }
  ...
  if (featureLower.includes("toggle") || featureLower.includes("switch")) {
    return {
      ...
      state: [`const [isToggled${index}, setIsToggled${index}] = useState(false);`],
      handlers: [
...
function generateReactCode(title: string, purpose: string, features: string[]): string {
  const componentName = title.replace(/[^a-zA-Z0-9]/g, '');
  const featureComponents = features.map((f, i) => generateFeatureComponent(f, i));
  ...
  const featureJSX = featureComponents.map((fc) => {
    switch(fc.type) {
      case 'input':
