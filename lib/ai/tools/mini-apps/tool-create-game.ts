...
const gameInput = z.object({
  ...
  genre: z.enum(["quiz", "memory", "flashcards", "estimation"]).default("quiz"),
  ...
  questions: z
    ...
    .min(1)
    .describe(
      "List of quiz questions. For non-quiz games used as content items."
    ),
});
...
export const createGame = tool({
  ...
  execute: ({ title, genre, difficulty, questions }) => {
